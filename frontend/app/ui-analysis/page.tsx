  ...
  return (
    <section className="mx-auto w-full max-w-6xl space-y-6">
      <header>
        ...
        {!projectLoading && personas.length === 0 && !error && (
          <div className="mt-3 flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            ...
            {selectedProject
              ? `No personas found in ${selectedProject.name}. Create one in the Personas tab first.`
      ...
      <form onSubmit={onSubmit} className="rounded-xl border border-slate-200 bg-white p-6 shadow ring-1 ring-black/5 space-y-5">
        ...
        <button
          type="submit"
          disabled={loading || personas.length === 0 || !selectedProjectId}
            ...
            {result.sample_ui?.html ? (
              <div>
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(result.sample_ui.html) }}
                />
              </div>
            ) : null}
